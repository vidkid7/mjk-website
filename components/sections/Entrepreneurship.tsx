    ...
    <section id="capabilities" className="relative overflow-hidden bg-[#071224] py-24 md:py-32">
      ...
      <div className="relative mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto mb-16 grid max-w-2xl grid-cols-1 gap-4 sm:grid-cols-3 md:gap-6">
          {content.stats.map((stat, i) => { const Icon = statIcons[i % statIcons.length]; return <div key={stat.label} className="border border-white/[0.08] bg-white/[0.04] p-5 text-center backdrop-blur-sm"><Icon size={18} className="mx-auto mb-2 text-emerald-300" /><div className="mb-1 text-2xl font-bold text-white md:text-3xl">{stat.value}</div><div className="text-xs font-semibold text-slate-500">{stat.label}</div></div> })}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative rounded-[1.5rem] border border-white/10 bg-white/[0.055] p-8 text-center backdrop-blur-sm md:p-10">
