...
export default function Entrepreneurship() {
  ...
  return (
    <section id="capabilities" className="public-section public-section--light relative overflow-hidden py-24 md:py-32">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0"><DhakaPattern className="opacity-[0.025]" /><div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-slate-300 to-transparent" /><div className="absolute -right-24 top-12 h-52 w-1/2 bg-gradient-to-l from-emerald-100/70 to-transparent" /><div className="absolute -left-24 bottom-12 h-52 w-1/2 bg-gradient-to-r from-blue-100/70 to-transparent" /></div>

      <div className="relative mx-auto max-w-6xl px-5 sm:px-8">
        <div className="mb-16 text-center">
          <motion.span initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-black uppercase tracking-[0.2em] text-emerald-700"><Rocket size={14} />Delivery Capability</motion.span>
          <motion.h2 initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="font-playfair text-3xl font-extrabold leading-tight tracking-tight text-slate-950 sm:text-5xl lg:text-6xl">{content.sectionTitle}</motion.h2>
          <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.12 }} className="mx-auto mt-5 max-w-3xl text-base leading-8 text-slate-600 md:text-lg">{content.sectionSubtitle}</motion.p>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto mb-16 grid max-w-2xl grid-cols-1 gap-4 sm:grid-cols-3 md:gap-6">
          {content.stats.map((stat, i) => { const Icon = statIcons[i % statIcons.length]; return <div key={stat.label} className="glass-inset rounded-2xl p-4 text-center"><Icon size={18} className="mx-auto mb-2 text-emerald-600" /><div className="mb-1 text-2xl font-bold text-slate-950 md:text-3xl">{stat.value}</div><div className="text-xs font-semibold text-slate-500">{stat.label}</div></div> })}
        </motion.div>
        ...
        <div className="mb-16 grid gap-5 md:grid-cols-3">
          {businessCards.map((card, i) => (
            <motion.div key={card.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 + i * 0.12 }} onMouseEnter={() => setHoveredCard(i)} onMouseLeave={() => setHoveredCard(null)} className="group relative">
              <motion.div whileHover={{ y: -8 }} className="glass-panel relative h-full overflow-hidden rounded-[1.75rem] p-6 md:p-8 transition hover:border-slate-200">
                ...
                <h4 className="relative mb-2 text-lg font-black text-slate-950">{card.title}</h4><p className="relative mb-4 text-sm leading-7 text-slate-600">{card.description}</p><div className="relative flex items-center gap-1.5 text-xs font-medium text-slate-500 transition group-hover:text-emerald-700"><span>Learn more</span><ArrowUpRight size={12} /></div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass-panel relative rounded-[1.75rem] p-6 text-center md:p-8">
          <div className="absolute left-1/2 top-0 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center border border-emerald-200 bg-emerald-50"><Quote size={16} className="text-emerald-600" /></div>
          <blockquote className="mx-auto mb-6 max-w-3xl pt-2 font-playfair text-xl italic leading-relaxed text-slate-700 md:text-2xl lg:text-3xl">&ldquo;{content.quoteText}&rdquo;</blockquote>
          <span className="text-sm font-medium tracking-wide text-emerald-700">{content.quoteAttribution}</span>
        </motion.div>
      </div>
    </section>
  )
