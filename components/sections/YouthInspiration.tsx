...
const legacyInsightTerms = ['if i can', 'young nepali', 'youth movement', 'youth bootcamp', 'leader', 'changemaker', 'nation', 'scholarship holder']

function isLegacyInsight(content: typeof defaultData) {
  const haystack = `${content.heading} ${content.description} ${content.ctaText} ${content.testimonials?.map(item => `${item.name} ${item.role} ${item.quote}`).join(' ')}`.toLowerCase()
  return legacyInsightTerms.some(term => haystack.includes(term))
}
...
export default function YouthInspiration() {
  ...
  const storedContent = useStoredData('youth', defaultData)
  const content = isLegacyInsight(storedContent) ? defaultData : storedContent
    ...
    <section id="insights" className="public-section public-section--light relative overflow-hidden px-4 py-24 md:py-32">
      ...
      <div className="relative mx-auto max-w-6xl">
        <div className="mb-16 text-center">
          <motion.span initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-black uppercase tracking-[0.2em] text-emerald-700"><Sparkles size={14} />Insights</motion.span>
          <motion.h2 initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.08 }} className="font-playfair text-3xl font-extrabold leading-tight tracking-tight text-slate-950 md:text-5xl lg:text-6xl">{content.heading}</motion.h2>
          <motion.p initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.16 }} className="mx-auto mt-5 max-w-3xl text-base leading-8 text-slate-600 md:text-lg">{content.description}</motion.p>
        </div>
          ...
          {cards.map((card, i) => {
            ...
            return (
              <motion.article key={card.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 + i * 0.1 }} onMouseEnter={() => setActiveCard(i)} onMouseLeave={() => setActiveCard(null)} className="glass-panel group relative h-full overflow-hidden rounded-[1.75rem] p-6 md:p-8 transition hover:-translate-y-1 hover:shadow-2xl hover:shadow-slate-900/10">
                ...
                <div className="mt-6 flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-slate-400 transition group-hover:text-emerald-700"><CheckCircle2 size={14} />Practical insight</div>
                <motion.div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${card.gradient}`} initial={{ scaleX: 0 }} animate={activeCard === i ? { scaleX: 1 } : { scaleX: 0 }} transition={{ duration: 0.3 }} style={{ transformOrigin: 'left' }} />
              </motion.article>
            )
          })}
        </div>
